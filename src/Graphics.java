...
/**
 * File: J2MEGraphics.java
 * Created on 2025 Nov 12, 15:22:14
 *
 * @author LWJGL2
 */
public class Graphics {
  public static final int HCENTER = 1;
    ...
    public void drawImage(Image img, int x, int y, int anchor) throws IllegalArgumentException, NullPointerException {
        ...
        x = ImageUtil.alignX(img.getWidth(null), x, anchor);
        y = ImageUtil.alignY(img.getHeight(null), y, anchor);
...
//        }
//        if (width > 0 && height > 0) {
//            var imageRegion = ((BufferedImage)src).getSubimage(x_src, y_src, width, height);
//            var transformedImage = ImageUtil.transformImage(imageRegion, transform);
//            x_dest = ImageUtil.alignX(transformedImage.getWidth(), x_dest, anchor);
//            y_dest = ImageUtil.alignY(transformedImage.getHeight(), y_dest, anchor);
...
//    }
    ...
    public void copyArea(int x_src, int y_src, int width, int height, int x_dest, int y_dest, int anchor) throws IllegalStateException, IllegalArgumentException {
        x_dest = ImageUtil.alignX(width, x_dest, anchor);
        y_dest = ImageUtil.alignY(height, y_dest, anchor);
