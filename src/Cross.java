...
class Cross {
    public int[] values = new int[]{10, 8, 6};
    ...
    public Cross() {
        ...
        try {
            recordStore = RecordStore.openRecordStore((String)"Fun2LinkCross", (boolean)true);
    ...
    public boolean isHighCross(int n) {
        for (int i = 0; i < 3; ++i) {
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public void addHighCross(int n) {
        for (int i = 0; i < 3; ++i) {
            if (n < this.values[i]) continue;
            for (int j = 2; j > i; --j) {
